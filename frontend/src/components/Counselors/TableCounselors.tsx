...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/counselors/counselorsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureCounselorsCols';
...
const TableSampleCounselors = ({
  ...
  const {
    ...
    loading,
    count,
    notify: counselorsNotify,
    refetch,
  } = useAppSelector((state) => state.counselors);
  ...
  useEffect(() => {
    if (counselorsNotify.showNotification) {
      notify(
        counselorsNotify.typeNotification,
        counselorsNotify.textNotification,
      );
    }
  }, [counselorsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/counselors/counselors-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {counselors && Array.isArray(counselors) && !showGrid && (
        <ListCounselors
...
export default TableSampleCounselors;
