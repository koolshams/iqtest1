...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/parents/parentsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureParentsCols';
...
const TableSampleParents = ({
  ...
  useEffect(() => {
    if (parentsNotify.showNotification) {
      notify(parentsNotify.typeNotification, parentsNotify.textNotification);
    }
  }, [parentsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/parents/parents-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {parents && Array.isArray(parents) && !showGrid && (
        <ListParents
          parents={parents}
          loading={loading}
...
export default TableSampleParents;
